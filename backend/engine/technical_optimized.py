...
def macd(closes):
    ...
    # Compute the 12/26-period EMAs incrementally instead of recomputing
    # them from scratch for every historical bar.
    k_fast=2/(12+1)
    k_slow=2/(26+1)
    ...
    # Advance the 12-period EMA to the latest bar.
    for x in vals[12:]:
        fast=x*k_fast + fast*(1-k_fast)

    # Advance the 26-period EMA to the latest bar.
    for x in vals[26:]:
        slow=x*k_slow + slow*(1-k_slow)

    # Build MACD history in one pass for the 9-period signal EMA.
    # The first MACD value is at index 25, when both EMAs exist.
    macd_values=[]
    f=sum(vals[:12])/12
    s=sum(vals[:26])/26
    for i in range(12,26):
        f=vals[i]*k_fast + f*(1-k_fast)
    macd_values.append(f-s)

    for i in range(26,len(vals)):
        f=vals[i]*k_fast + f*(1-k_fast)
        s=vals[i]*k_slow + s*(1-k_slow)
        macd_values.append(f-s)
