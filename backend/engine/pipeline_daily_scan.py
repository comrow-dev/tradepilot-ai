...
def timeframe_data(fh, symbol):
    ...
    # Normal scan uses the daily timeframe because TradePilot's current
    # scoring engine reads the daily technical snapshot. Intraday data is
    # intentionally deferred so the scan does not exceed the free
    # Twelve Data API credit rate. It can be added later as a separate
    # batched/deep-analysis layer without slowing the normal scan.
    try:
...
def candidate(fh, symbol, market):
    ...
    # TradePilot Intelligence körs separat för fördjupad analys.
    # Den ska inte blockera den vanliga marknadsskanningen.
    intelligence = {
        "ok": False,
        "symbol": symbol,
        "count": 0,
...
def scan_market(
    ...
    # Standard: analysera 3 kandidater i den vanliga snabba skanningen.
